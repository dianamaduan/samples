<?php 
  ...
  if(isset($_SESSION["genre.id"])) {
    $id = $_SESSION["genre.id"];
    $query = $db->query("SELECT m.poster,g.genre,m.title,m.date FROM movies m,movgenre mg,genre g where m.mov_id=mg.mov_id and mg.genre_id=g.genre_id and g.genre_id=$id order by m.date");
    if($query->num_rows>0) {
...
<section class="films"><hr>
	 <?php
      $query = $db->query("SELECT * from genre where genre_id=$id ");
foreach ($query as $print) { ?> 

	<p class="text4"><?php echo $print['genre']; ?></p><hr> <?php } ?>
	<div class="latest">
		
	 <?php
      $query = $db->query("SELECT m.poster,g.genre,m.title,m.date,m.mov_id FROM movies m,movgenre mg,genre g where m.mov_id=mg.mov_id and mg.genre_id=g.genre_id and g.genre_id=$id order by m.date");
foreach ($query as $print) { ?> 
...
<h1 style="font-size: 15px; color:gray;font-weight: 500; "><?php echo $print['date'] ; ?></h1></div>
<?php } ?>
